...
func ga() func() {
	...
	return func() {
		...
		for nodeXY := 0; nodeXY < len(best.genes); nodeXY++ {
			nodeX.WriteString(strconv.FormatFloat(best.genes[nodeXY].x, 'g', 10, 64))
			nodeX.WriteByte(',')
			nodeY.WriteString(strconv.FormatFloat(best.genes[nodeXY].y, 'g', 10, 64))
			nodeY.WriteByte(',')
		}
		js.Global().Call("updateNode", js.ValueOf(nowGen), js.ValueOf(minGen), js.ValueOf((best.fitness-15780)/15780*100),
